...
# Build full input row using X_train.csv schema
original = dataset.iloc[0:1].copy()

for col, value in user_inputs.items():
    original[col] = value

# Force exact same column order as training data
original = original[dataset.columns]
